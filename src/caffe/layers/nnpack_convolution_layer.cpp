...
nnp_convolution_transform_strategy nnp_kts(
    NNPACKConvolutionParameter_KernelTransformStrategy kts) {
  auto convolution_transform_strategy =
  ...
  return convolution_transform_strategy;
}
...
void caffe_nnp_convolution_forward(
  ...
  const nnp_size output_subsampling = {.width = static_cast<size_t>(stride.cpu_data()[0]),
                                      .height = static_cast<size_t>(stride.cpu_data()[1])};
